...
function SupportManagement() {
  ...
  const handleAwaitingConfirm = async () => {
    ...
    // Update form state (for modal use case)
    ...
    // If called from table dropdown (not from inside edit modal), save directly
    if (editingId && !showModal) {
      try {
        ...
        await axios.put(`/api/support-tickets/${editingId}`, fd, { headers: hdrs });
        fetchAll();
      } catch(e) { console.error(e); }
    }
  };
        ...
        {loading ? <div className="loading">{t('loading')}</div> : (
          <div className="table-container" style={{ overflowX:'visible' }}>
            <table className="table" style={{ tableLayout:'fixed', width:'100%' }}>
              <thead>
                <tr>
                  <th onClick={() => handleSort('id')} style={{ cursor:'pointer', userSelect:'none' }}><SortIcon column="id" sortBy={sortBy} sortOrder={sortOrder} />#</th>
                  <th onClick={() => handleSort('created_by_name')} style={{ cursor:'pointer', userSelect:'none' }}><SortIcon column="created_by_name" sortBy={sortBy} sortOrder={sortOrder} />{t('created_by')}</th>
                  <th onClick={() => handleSort('owner_name')} style={{ cursor:'pointer', userSelect:'none' }}><SortIcon column="owner_name" sortBy={sortBy} sortOrder={sortOrder} />{t('owner')}</th>
                  ...
                  <th onClick={() => handleSort('priority')} style={{ cursor:'pointer', userSelect:'none' }}><SortIcon column="priority" sortBy={sortBy} sortOrder={sortOrder} />{t('priority')}</th>
                  <th onClick={() => handleSort('status')} style={{ cursor:'pointer', userSelect:'none' }}><SortIcon column="status" sortBy={sortBy} sortOrder={sortOrder} />{t('status')}</th>
                  <th onClick={() => handleSort('created_at')} style={{ cursor:'pointer', userSelect:'none' }}><SortIcon column="created_at" sortBy={sortBy} sortOrder={sortOrder} />{t('date')}</th>
                  <th style={{ width:'110px', minWidth:'110px' }}>{t('actions')}</th>
                </tr>
              </thead>
              <tbody>
                {sorted.length === 0 ? (
                  <tr><td colSpan="9" className="text-center">{t('no_tickets')}</td></tr>
                ) : sorted.map(tk => (
                  <tr key={tk.id}>
                    <td>#{tk.id}</td>
                    <td style={{ wordBreak:'break-word' }}>{tk.created_by_name||'-'}</td>
                    <td style={{ wordBreak:'break-word' }}>{tk.owner_name||'-'}</td>
                    ...
                    <td>
                      <div style={{ display:'flex', flexDirection:'column', gap:'0.3rem', alignItems:'stretch' }}>
                        <button title={t('edit')||'Edit'} onClick={() => openEdit(tk)} style={{ padding:'0.3rem 0.5rem', background:'#f1f5f9', color:'#475569', border:'1px solid #cbd5e1', borderRadius:'6px', cursor:'pointer', fontSize:'0.82rem', fontWeight:600, whiteSpace:'nowrap' }}>✏️ {t('edit')||'Edit'}</button>
                        <button title={t('history')||'History'} onClick={() => openHistory(tk)} style={{ padding:'0.3rem 0.5rem', background:'#7c3aed', color:'white', border:'none', borderRadius:'6px', cursor:'pointer', fontSize:'0.82rem', fontWeight:700, whiteSpace:'nowrap' }}>📋 {t('history')||'History'}</button>
                        <button title={t('send_product')||'Send Product'} onClick={() => { setEditingId(tk.id); setSendProductForm({ product_id:'', product_name:'', quantity:1 }); setShowSendProduct(true); }} style={{ padding:'0.3rem 0.5rem', background:'#ea580c', color:'white', border:'none', borderRadius:'6px', cursor:'pointer', fontSize:'0.82rem', fontWeight:700, whiteSpace:'normal', lineHeight:'1.3', textAlign:'center' }}>📦 {t('send_product')||'Send Product'}</button>
                        <select
                          value={tk.status}
                          onChange={e => {
                            ...
                            if (newStatus === 'awaiting_customer') {
                              setEditingId(tk.id);
                              setForm({...tk, status: 'awaiting_customer'});
                              setAwaitingForm({ channel:'', note:'', deadline:'' });
                              setShowAwaitingPopup(true);
                            } else {
                              const fd = new FormData();
                              fd.append('subject', tk.subject||'');
                              fd.append('description', tk.description||'');
                              fd.append('status', newStatus);
                              fd.append('priority', tk.priority||'medium');
                              fd.append('customer_id', tk.customer_id||'');
                              fd.append('customer_name', tk.customer_name||'');
                              fd.append('product_id', tk.product_id||'');
                              fd.append('product_name', tk.product_name||'');
                              fd.append('owner_id', tk.owner_id||'');
                              const hdrs = { Authorization: `Bearer ${sessionStorage.getItem('token')}`, 'Content-Type': 'multipart/form-data' };
                              axios.put(`/api/support-tickets/${tk.id}`, fd, { headers: hdrs })
                                .then(() => fetchAll())
                                .catch(err => console.error(err));
                            }
                          }}
                          style={{ padding:'0.3rem 0.4rem', border:'1px solid #cbd5e1', borderRadius:'6px', fontSize:'0.78rem', fontWeight:600, cursor:'pointer', background:'white', color:'#374151', width:'100%' }}
                        >
                        ...
                        <button title={t('delete')||'Delete'} onClick={() => handleDelete(tk.id)} style={{ padding:'0.3rem 0.5rem', background:'#fee2e2', color:'#dc2626', border:'1px solid #fca5a5', borderRadius:'6px', cursor:'pointer', fontSize:'0.82rem', fontWeight:600, whiteSpace:'nowrap' }}>🗑️ {t('delete')||'Delete'}</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {showHistory && (
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth:'620px', width:'95%' }}>
            <div className="modal-header">
              <h3 className="modal-title">📋 {t('history')} — {t('ticket')||'#'} #{historyTicket?.id} | {historyTicket?.subject}</h3>

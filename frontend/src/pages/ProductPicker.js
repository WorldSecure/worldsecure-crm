        ...
        return (
          <div key={parent.id}>
            <div onClick={() => toggleParent(parent.id)}
              style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: isMobile ? '0.75rem 1rem' : '0.5rem 0.75rem', cursor: 'pointer', background: '#fffbf0', borderBottom: '1px solid #f5f0e0', fontSize: isMobile ? '1rem' : '0.88rem', fontWeight: 600, color: '#444', minHeight: isMobile ? '52px' : 'auto' }}>
            ...
            {expanded && variants.map(variant => (
              <div key={variant.id} onClick={() => handleSelect(variant)}
                style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: isMobile ? '0.65rem 1rem 0.65rem 2.5rem' : '0.45rem 0.75rem 0.45rem 2rem', cursor: 'pointer', borderBottom: '1px solid #fafafa', fontSize: isMobile ? '0.95rem' : '0.85rem', background: String(variant.id) === String(value) ? '#e8f5e9' : 'white', minHeight: isMobile ? '48px' : 'auto' }}>
      ...
      {isMobile && open && (
        <div>
          ...
          <div onClick={handleClose}
            style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.45)', zIndex: 9998 }} />
          {/* Sheet */}
          <div

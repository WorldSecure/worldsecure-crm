  ...
  try {
    ...
    for (const product of parents) {
      try {
        ...
        await dbRun("UPDATE products SET name_he=?, name_pt=?, translation_done=1, meta_updated_at=datetime('now') WHERE id=?", [t.he, t.pt, product.id]);
    ...
    for (const variant of variants) {
      try {
        ...
        if (parentRow && parentRow.name_he && parentRow.name_pt) {
          ...
          await dbRun("UPDATE products SET name_he=?, name_pt=?, translation_done=1, meta_updated_at=datetime('now') WHERE id=?",
            [parentRow.name_he + (label ? ' ' + label : ''), parentRow.name_pt + (label ? ' ' + label : ''), variant.id]);
        } else {
          const t = await callAnthropicAPI(
            'Translate this product name. Return ONLY a JSON object: {"he":"...","pt":"..."}\n\nProduct name: ' + variant.name
          );
          await dbRun("UPDATE products SET name_he=?, name_pt=?, translation_done=1, meta_updated_at=datetime('now') WHERE id=?", [t.he, t.pt, variant.id]);
          await delay(600);
        }
        details.products++; count++;
            ...
            const insertVariant = (combo, index) => {
              ...
              db.get('SELECT id, parent_id FROM products WHERE sku = ?', [baseVariantSku], (err, existing) => {
                if (existing && existing.parent_id && existing.parent_id !== parentId) {
                  // SKU תפוס על ידי אב אחר — מצא suffix פנוי
                  let suffix = 1;
                  const findSuffix = () => {
                    const candidate = `${baseVariantSku}${suffix}`;
                    db.get('SELECT id FROM products WHERE sku = ?', [candidate], (err, row) => {
                      if (row) { suffix++; findSuffix(); return; }
                      db.run(
                  ...
                  // SKU קיים בלי אב — קשר לאב הנוכחי
                  db.run('UPDATE products SET parent_id=? WHERE id=?', [parentId, existing.id],
                    () => insertVariant(combos[index + 1], index + 1));
                } else if (!existing) {
                  // SKU לא קיים — צור חדש
                  db.run(
